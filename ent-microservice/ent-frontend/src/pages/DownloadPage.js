...
//     try {
//       const response = await fetch('http://localhost:8004/list-courses/', {
//         method: 'GET',
//         headers: {
...
//     try {
//       const response = await fetch(`http://localhost:8004/download/${encodeURIComponent(bucket_name)}/${encodeURIComponent(object_name)}`, {
//         method: 'GET',
//         headers: {
//           Authorization: `Bearer ${user.token}`,
//         },
...
import { Link } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext';
import { getStudentByEmail, getFiliereById } from '../services/studentService';
import {
  FaBook,
  FaUsers,
  FaQuestionCircle,
...
const DownloadPage = () => {
  const { user } = useAuth();
  const [courses, setCourses] = useState({ filiere: '', modules: [] });
  ...
  useEffect(() => {
    const fetchFiliere = async () => {
      if (!user?.email) {
        setError('User email is not available. Please log in.');
        return;
      }
      ...
      try {
        ...
        if (!studentByEmail?.filiere_id) {
          throw new Error('Student filiere ID not found.');
        }

        const filiereData = await getFiliereById(studentByEmail.filiere_id, { _t: Date.now() });
        setFiliere(filiereData.name || 'N/A');
      } catch (err) {
        console.error('Error fetching filiere:', err);
        setError('Failed to load filiere information. Please ensure your profile is complete.');
      }
    };
  ...
  // Fetch courses when filiere is available
  useEffect(() => {
    const fetchCourses = async () => {
      if (!user?.token || !filiere || filiere === 'N/A') {
        if (!user?.token) setError('Authentication token is missing. Please log in again.');
        if (!filiere || filiere === 'N/A') setError('Filiere information is missing.');
        return;
      }
      ...
      try {
        const response = await fetch(`http://localhost:8004/courses/?filiere=${encodeURIComponent(filiere)}`, {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        ...
        if (!response.ok) {
          ...
          if (response.status === 400) {
            throw new Error('Invalid filiere provided.');
          }
          throw new Error(data.detail || `Failed to fetch courses (status: ${response.status}).`);
        }
        ...
        setError(err.message || 'An unexpected error occurred while fetching courses. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDownload = async (bucket_name, object_name) => {
    if (!user?.token || !filiere) {
      setError('Authentication token or filiere is missing. Please log in again.');
      return;
    }

    try {
      const response = await fetch(`http://localhost:8004/download/${encodeURIComponent(bucket_name)}/${encodeURIComponent(object_name)}?filiere=${encodeURIComponent(filiere)}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      ...
      setError(err.message || 'An unexpected error occurred while downloading the file. Please try again later.');
    }
  };
  ...
  const toggleElement = (moduleName, elementName) => {
    setOpenElements((prev) => ({
      ...prev,
      [`${moduleName}-${elementName}`]: !prev[`${moduleName}-${elementName}`]
    }));
  };
    ...
    <div className="bg-gray-50 text-gray-800 min-h-screen flex flex-col">
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        ...
        <main className="flex-1 p-4 sm:p-6 md:p-8 bg-white m-2 sm:m-4 rounded-lg shadow-sm">
          <div className="welcome-banner bg-est-blue text-white p-4 sm:p-6 md:p-8 rounded-lg mb-4 sm:mb-6 md:mb-8 text-center">
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold mb-2 sm:mb-4">Accéder à Vos Cours</h1>
            <p className="text-sm sm:text-base md:text-lg opacity-90 mb-4 sm:mb-6">Explorez les ressources pédagogiques pour votre filière : {filiere || 'Chargement...'}</p>
          </div>

          <div className="courses-list bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100">
            <h2 className="text-est-blue text-base sm:text-lg font-semibold mb-4">Ressources Disponibles</h2>
            {error && (
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {courses.modules.map((module, moduleIndex) => (
                  <div key={moduleIndex} className="border border-gray-200 rounded-lg">
                    <button
                      onClick={() => toggleModule(module.name)}
                      className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 text-left text-sm sm:text-base font-medium text-gray-800"
                    >
                      <span>{module.name}</span>
                      {openModules[module.name] ? <FaChevronDown /> : <FaChevronRight />}
                    </button>
                    {openModules[module.name] && (
                      <div className="p-4 space-y-4">
                        {module.elements.map((element, elementIndex) => (
                          <div key={elementIndex} className="ml-4 border-l-2 border-gray-200 pl-4">
                            <button
                              onClick={() => toggleElement(module.name, element.name)}
                              className="w-full flex justify-between items-center py-2 text-left text-xs sm:text-sm font-medium text-gray-700"
                            >
                              <span>{element.name}</span>
                              {openElements[`${module.name}-${element.name}`] ? <FaChevronDown /> : <FaChevronRight />}
                            </button>
                            {openElements[`${module.name}-${element.name}`] && (
                              <div className="mt-2 ml-4">
                                {element.files.length === 0 ? (
                                  <p className="text-gray-600 text-xs sm:text-sm">Aucun fichier disponible.</p>
                                ) : (
                                  <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-200 hidden sm:table">
                                      <thead className="bg-gray-50">
                                    ...
                                    <div className="sm:hidden space-y-4">
                                      {element.files.map((file, fileIndex) => (
                                        <div key={fileIndex} className="bg-gray-50 p-4 rounded-lg shadow-sm">
                                          <p className="text-xs font-medium text-gray-700 break-words"><span className="font-semibold">Titre:</span> {file.metadata.titre}</p>
                                          <p className="text-xs text-gray-600 break-words"><span className="font-semibold">Nom:</span> {file.object_name}</p>
                                          <div className="mt-2">
                                            <button
                                              onClick={() => handleDownload(file.bucket_name, file.object_name)}
                                              className="text-est-blue hover:text-blue-900 text-xs flex items-center"
