...
from app.database.models import Element, Module
from app.schemas.elements import ElementCreateSchema, ElementUpdateSchema, ElementSchema
...
def get_elements_by_module(module_id: UUID, skip: int = 0, limit: int = 100, search: str = '') -> Dict:
    try:
        # Validate module_id
        if not Module.objects(id=module_id).first():
            raise HTTPException(status_code=400, detail="Invalid module_id")
        elements = Element.objects(module_id=module_id).allow_filtering()
        total = elements.count()
        elements_list = list(elements)
        if search:
            elements_list = [e for e in elements_list if search.lower() in (e.name or '').lower()]
            total = len(elements_list)
        elements_list = elements_list[skip:skip + limit]
        result = {
            "elements": [ElementSchema(**element_to_dict(e)) for e in elements_list],
            "total": total
        }
        return result
