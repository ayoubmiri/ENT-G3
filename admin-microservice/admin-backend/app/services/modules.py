...
from app.crud.modules import (
    ...
    update_module,
    delete_module,
    get_modules_by_filiere,  # Import the new CRUD function
)
...
def get_modules_by_filiere_service(filiere_id: UUID, skip: int = 0, limit: int = 100, search: str = '') -> List[ModuleInDB]:
    """
    Retrieve modules by filiere ID with pagination and optional search.

    Args:
        filiere_id (UUID): The ID of the filiere to filter modules.
        skip (int): Number of records to skip (default: 0).
        limit (int): Maximum number of records to return (default: 100).
        search (str): Optional search term to filter modules by name (default: '').

    Returns:
        List[ModuleInDB]: List of modules for the specified filiere.
    """
    result = get_modules_by_filiere(filiere_id=filiere_id, skip=skip, limit=limit, search=search)
    return result["modules"]
